...
class ContactsCleaner:
    ...
    def clean_and_transform(self):
        ...
        t1_contacts_df = pd.read_excel(
            self.path, sheet_name=self.tier_sheets["tier1"], header=0, engine="openpyxl"
        )

        t2_contacts_df = pd.read_excel(
            self.path, sheet_name=self.tier_sheets["tier2"], header=0, engine="openpyxl"
        )
...
class EventsCleaner:
    ...
    def clean_and_transform(self, lpd_event, mrc_event):
        ...
        marketing_df = pd.merge(
            lpd_participants, mrc_participants, on="E-mail", how="outer"
        )
        ...
        marketing_df = marketing_df[["Name", "E-mail", "LPD", "19_MRC"]].reset_index(
            drop=True
        )
